...
class announcedaily3(Cog):
    ...
    @commands.group(name="announcedaily3", aliases=["annd33"])
    @checks.mod_or_permissions(administrator=True)
    @commands.guild_only()
    async def _ad(self, ctx: commands.Context):
        """
        Base command for managing announcedaily3 settings

        Do `[p]help annd33 <subcommand>` for more details
        """
        pass
    ...
    async def delmsg3(self, ctx: commands.Context, index: int):
        ...
        Must provide the index of the message, which can be found by using `[p]annd33 listmsg2`
        """
    ...
    @_ad.command()
    async def delimg3(self, ctx: commands.Context, filename: str):
        """
    ...
    async def check_day(self):
        while True:
            ...
            if self is not self.bot.get_cog("announcedaily3"):
...
# [p]delimg3 - Remove image from pool
# [p]settime3 <x> - S

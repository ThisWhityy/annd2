...
class announcedaily4(Cog):
    ...
    @checks.guildowner()
    @commands.guild_only()
    async def runannounce4(self, ctx: commands.Context):
        """Manually run the daily announcement"""

        await self.send_announcements4()
        await ctx.send("Success")

    @_ad.command()
    async def setchannel4(self, ctx: commands.Context, channel: discord.TextChannel = None):
        """
    ...
    @_ad.command()
    async def addmsg4(self, ctx: commands.Context, *, msg):
        """
    ...
    @_ad.command()
    async def addimg4(self, ctx: commands.Context, filename=None):
        """
    ...
    @_ad.command()
    async def delmsg4(self, ctx: commands.Context, index: int):
        """
    ...
    @_ad.command()
    async def delimg4(self, ctx: commands.Context, filename: str):
        """
    ...
    @_ad.command()
    async def settime4(self, ctx: commands.Context, minutes_from_now: int):
        """
    ...
    async def send_announcements4(self):
    ...
    async def check_day(self):
        while True:
            ...
            if self is not self.bot.get_cog("announcedaily4"):
...
# [p]setchannel4 #channelname - Set the announcement channel per server
# [p]addmsg4 <message goes here> - Adds a msg to the pool
# [p]addimg4 http://imgurl.com/image.jpg - Adds an image to the pool
# [p]listmsg2 - Lists all messages in the pool
# [p]listimgs3 - Unsure about this one, but would probably just post all the images
# [p]delmsg4 - Remove msg from pool
# [p]delimg4 - Remove image from pool
# [p]settime4 <x> - S

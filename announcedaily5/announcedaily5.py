...
class announcedaily5(Cog):
    ...
    @commands.group(name="announcedaily5", aliases=["annd55"])
    @checks.mod_or_permissions(administrator=True)
    @commands.guild_only()
    async def _ad(self, ctx: commands.Context):
        """
        Base command for managing announcedaily5 settings

        Do `[p]help annd55 <subcommand>` for more details
        """
        pass
    ...
    @checks.guildowner()
    @commands.guild_only()
    async def runannounce5(self, ctx: commands.Context):
        """Manually run the daily announcement"""

        await self.send_announcements5()
        await ctx.send("Success")

    @_ad.command()
    async def setchannel5(self, ctx: commands.Context, channel: discord.TextChannel = None):
        """
    ...
    @_ad.command()
    async def addmsg5(self, ctx: commands.Context, *, msg):
        """
    ...
    @_ad.command()
    async def addimg5(self, ctx: commands.Context, filename=None):
        """
    ...
    @_ad.command()
    async def delmsg5(self, ctx: commands.Context, index: int):
        """
        Remove a message from the announcement pool

        Must provide the index of the message, which can be found by using `[p]annd55 listmsg2`
        """
    ...
    @_ad.command()
    async def settime5(self, ctx: commands.Context, minutes_from_now: int):
        """
    ...
    async def send_announcements5(self):
    ...
    async def check_day(self):
        while True:
            ...
            if self is not self.bot.get_cog("announcedaily5"):
...
# [p]setchannel5 #channelname - Set the announcement channel per server
# [p]addmsg5 <message goes here> - Adds a msg to the pool
# [p]addimg5 http://imgurl.com/image.jpg - Adds an image to the pool

...
class announcedaily6(Cog):
    ...
    @commands.group(name="announcedaily6", aliases=["annd66"])
    @checks.mod_or_permissions(administrator=True)
    @commands.guild_only()
    async def _ad(self, ctx: commands.Context):
        """
        Base command for managing announcedaily6 settings

        Do `[p]help annd66 <subcommand>` for more details
        """
        pass
    ...
    @checks.guildowner()
    @commands.guild_only()
    async def runannounce6(self, ctx: commands.Context):
        """Manually run the daily announcement"""

        await self.send_announcements6()
        await ctx.send("Success")

    @_ad.command()
    async def setchannel6(self, ctx: commands.Context, channel: discord.TextChannel = None):
        """
    ...
    @_ad.command()
    async def addmsg6(self, ctx: commands.Context, *, msg):
        """
    ...
    @_ad.command()
    async def addimg6(self, ctx: commands.Context, filename=None):
        """
    ...
    @_ad.command()
    async def listimgs3(self, ctx: commands.Context):
        """
    ...
    @_ad.command()
    async def delmsg6(self, ctx: commands.Context, index: int):
        """
        Remove a message from the announcement pool

        Must provide the index of the message, which can be found by using `[p]annd66 listmsg2`
        """
    ...
    @_ad.command()
    async def delimg6(self, ctx: commands.Context, filename: str):
        """
    ...
    @_ad.command()
    async def settime6(self, ctx: commands.Context, minutes_from_now: int):
        """
    ...
    async def send_announcements6(self):
    ...
    async def check_day(self):
        while True:
            ...
            if self is not self.bot.get_cog("announcedaily6"):
...
# [p]setchannel6 #channelname - Set the announcement channel per server
# [p]addmsg6 <message goes here> - Adds a msg to the pool
# [p]addimg66 http://imgurl.com/image.jpg - Adds an image to the pool
# [p]listmsg2 - Lists all messages in the pool
# [p]listimgs3 - Unsure about this one, but would probably just post all the images
# [p]delmsg6 - Remove msg from pool
# [p]delimg6 - Remove image from pool
# [p]settime6 <x> - S
